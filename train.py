...
def load_model(Model, params, checkpoint_path=""):
    ...
    if checkpoint_path == "":
        ...
        model = Model.load_from_checkpoint(
            checkpoint_path, UNet_params=params["model"], params=p
        )
    return model
...
def get_trainer(gpus, params):
    ...
    if params["train"]["early_stopping"]:
        early_stop_callback = EarlyStopping(
            monitor="val_loss", patience=params["train"]["patience"], mode="min"
        )
...
def train(params, gpus, mode, checkpoint_path, model="UNetModel"):
    ...
    if model == "SmatUnet":
        ...
        from models.smaat_pablo import UNet_Lightning as UNetModel

    model = load_model(UNetModel, params, checkpoint_path)
    # ------------
    # trainer
    # ------------
...
def set_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="./configurations/config_basline.yaml",
