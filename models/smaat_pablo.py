...
# models
from models.SmaAt_UNet import SmaAt_UNet as Base_UNET3D  # 3_3_2 model selection
...
class UNet_Lightning(L.LightningModule):
    def __init__(self, UNet_params: dict, params: dict, **kwargs):
        super().__init__()
        ...
        self.model = Base_UNET3D(
            in_channels=self.in_channels, n_classes=self.out_channels
        )
        ...
        t = (
            f"============== n_workers: {params['n_workers']} | batch_size: {params['batch_size']} \n"
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_hat = (
            upsample(y_hat) if self.params.get("size_target_center") != 252 else y_hat
        )
    ...
    def test_step(self, batch, batch_idx, phase="test"):
        ...
        if self.plot_results:
            ...
            self.plot_batch(x, y, y_hat, metadata, title, phase, vmax=1.0)
        # LOGGING
    ...
    def predict_step(self, batch, batch_idx, phase="predict"):
        ...
        if self.plot_results:
            self.plot_batch(
                ...
                y_hat,
                metadata,
                f"batch: {self.val_batch} | prediction results",
                phase,
                vmax=1.0,
            )
    ...
    def plot_batch(self, xs, ys, y_hats, metadata, loss, phase, vmax=0.01, vmin=0):
        ...
        for i in range(len(xs)):
            ...
            fig = plot_sequence(
                ...
                vmax=vmax,
                vmin=vmin,
                channel=self.in_channel_to_plot,
                title=loss,
            )
            figures.append(fig)
            # save individual image to tensorboard
            self.logger.experiment.add_figure(
                f"preds_{self.trainer.global_step}_{self.val_batch}_{i}", fig
            )
