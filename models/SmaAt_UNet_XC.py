...
class SmaAt_UNet(nn.Module):
    def __init__(self, in_channels, n_classes, kernels_per_layer=2, bilinear=True, reduction_ratio=16):
        ...
        self.xc_att1 = XCAttention(dim=64, heads=8, scale=8)  # XCAttention alongside CBAM

        self.down1 = DownDS(64, 128, kernels_per_layer=kernels_per_layer)
        self.cbam2 = CBAM(128, reduction_ratio=reduction_ratio)
        self.xc_att2 = XCAttention(dim=128, heads=8, scale=8)  # XCAttention alongside CBAM

        self.down2 = DownDS(128, 256, kernels_per_layer=kernels_per_layer)
        self.cbam3 = CBAM(256, reduction_ratio=reduction_ratio)
        self.xc_att3 = XCAttention(dim=256, heads=8, scale=8)  # XCAttention alongside CBAM

        self.down3 = DownDS(256, 512, kernels_per_layer=kernels_per_layer)
        self.cbam4 = CBAM(512, reduction_ratio=reduction_ratio)
        self.xc_att4 = XCAttention(dim=512, heads=8, scale=8)  # XCAttention alongside CBAM

        factor = 2 if self.bilinear else 1
        self.down4 = DownDS(512, 1024 // factor, kernels_per_layer=kernels_per_layer)
        self.cbam5 = CBAM(1024 // factor, reduction_ratio=reduction_ratio)
        self.xc_att5 = XCAttention(dim=1024 // factor, heads=8, scale=8)  # XCAttention alongside CBAM
    ...
    def forward(self, x):
        ...
        x1_att = self.xc_att1(x1_cbam)

        x2 = self.down1(x1_att)
        x2_cbam = self.cbam2(x2)
        x2_att = self.xc_att2(x2_cbam)

        x3 = self.down2(x2_att)
        x3_cbam = self.cbam3(x3)
        x3_att = self.xc_att3(x3_cbam)

        x4 = self.down3(x3_att)
        x4_cbam = self.cbam4(x4)
        x4_att = self.xc_att4(x4_cbam)

        x5 = self.down4(x4_att)
        x5_cbam = self.cbam5(x5)
        x5_att = self.xc_att5(x5_cbam)
